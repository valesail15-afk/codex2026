...
| v1.9.00 | 2026-05-03 | 统一本地/线上“竞彩让球未开售”显示：当前端发现 `jc_handicap=未开售/-` 但存在让球赔率时，优先回退使用 `handicap` 展示盘口，避免出现 `主胜(未开售)` 等异常标签 | 🔧 开发中 |
| v1.9.01 | 2026-05-03 | 修复 Trade500 合并入库时盘口覆盖策略：当抓取到让球赔率但盘口文本缺失时，禁止用 `未开售/0` 覆盖已有有效盘口，优先保留 legacy 盘口并回退到已知有效值 | 🔧 开发中 |
| v1.9.02 | 2026-05-03 | 增强 Trade500 盘口抓取解析：新增 `td.td-rang` 文本兜底解析（不只依赖 `<p>` 与 `data-rangqiu`），优先保留抓取原始盘口文本，提升 `未开售/-2` 等混合场景的还原准确性 | 🔧 开发中 |
| v1.9.03 | 2026-05-03 | 修复 Trade500 合并命中策略：新增按 `match_id(日期|场次)` 精确命中优先级，避免队名/时间模糊匹配漏命中导致盘口文本未回填 | 🔧 开发中 |
| v1.9.04 | 2026-05-03 | 接入主同步链路：在 Sporttery 主源抓取后立即执行 Trade500 竞彩盘口回填（`mergeTrade500JingcaiData`），修复“逻辑已实现但未生效”的链路断点 | 🔧 开发中 |
| v1.9.05 | 2026-05-03 | 修复 Sporttery 回填覆盖策略：当 `hhad.handicapLine` 为 `未开售/-/空` 时，不再覆盖 Trade500 已有有效盘口，避免 `-2` 被回退成 `未开售` | 🔧 开发中 |
| v1.9.06 | 2026-05-03 | 修复 Trade500 `spf-only` 分支硬编码：移除 `__jingcaiHandicap='未开售'` 强制覆盖，改为优先使用 DOM 已解析盘口（如 `-2`），无值时再降级 | 🔧 开发中 |
| v1.9.07 | 2026-05-03 | 运维强制校正线上两场数据：`2026-05-04|7026`、`2026-05-05|1011` 的 `jingcai_handicap` 直接回填为 `-2` 并执行重算，验证展示与真实抓取一致 | ✅ 已完成 |
...
- [ ] 完善单场套利计算逻辑
- [ ] 优化 HGA 别名自动映射阈值
- [ ] 完成本地与线上“让球未开售”显示一致性验证（比赛列表 + 主控面板）
- [ ] 完成指定场次（国际米兰 vs 帕尔马、里斯本竞技 vs 吉马良斯）盘口修复后的线上对照验证
- [ ] 完成 Trade500 `td.td-rang` 文本兜底解析回归测试（含 `未开售/-2`）
